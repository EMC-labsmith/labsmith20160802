...
def hello(req):
    ...
    devices5 = Device.objects.filter(name__startswith="BR").order_by("name")
...
def reserve_dev(id, currentuser):
    ...
    #     return msg
    # else: 
        uselog = UsageLog.objects.filter(machineName=device.name)&UsageLog.objects.filter(user=currentuser.get_full_name())&UsageLog.objects.filter(port=device.port)
        for log in uselog:
            if log.isUse == 'f':
                log.isUse = 't'
                log.save()
    ...
    # print "test"
        if device.port:
            msg = "%s %s is owned by %s now." % (device.name, device.port, currentuser.get_full_name())
        else:
...
def IPResult(req):
    ...
    ip1 = Device.objects.filter(name__startswith="OB").order_by("name")
